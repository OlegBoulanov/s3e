...
namespace com.amazon.s3
{
    ...
    public partial class AWSAuthConnection
    {
		...
		public GetResponse getIfModifiedSince(string bucket, string key, DateTime ifModifiedSince, bool asStream)
		{
			WebRequest request = makeRequest("GET", encodeKeyForSignature(key), null, bucket);
			if(request is HttpWebRequest)
			{
				HttpWebRequest httpRequest = request as HttpWebRequest;
				httpRequest.IfModifiedSince = ifModifiedSince;
			}
			return new GetResponse(request, asStream);
		}
